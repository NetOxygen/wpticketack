...
.affiche {
    background-image: url("<?php echo tkt_img_proxy_url($s->movies()[$main_movie_index]->opaque()["posters"][0]["url"], 755, 885) ?>");
    background-repeat:no-repeat;
...
<?php if (!(is_null($s))) : ?>
    <tr class="noborder" height="80%">
        <td class="heure_seance" width="499" rowspan="2">
            <h3>
                Heure<br/><?php echo wp_date('H:i',$s->start_at()->getTimestamp()); ?>
            </h3>
        </td>
        <td id="cadre_countdown" class="start_in" width="666" >
            ...
            <h3 id="age">
                Age : +<?php if(!empty($s->movies()[0]->opaque()["l_min_age"])) echo $s->movies()[0]->opaque()["l_min_age"]; else echo 0; ?></h3><h3 id="version"><?php echo$s->opaque()["version"]; ?>
            </h3>
        </td>
    ...
    <tr class="noborder">
        <th class="cadre_film" colspan="2">
            <?php echo $s->localized_title_or_original($language) ?>
        </th>
    </tr>
<?php endif; ?>
</table>
...
<script>
    <?php if (!(is_null($s))) : ?>
        var countDownDate    = <?php echo $s->start_at()->getTimestamp(); ?> * 1000;
        var now              = <?php echo time(); ?> * 1000;
        var reloadDate       = <?php echo $time_to_reload->getTimestamp(); ?> * 1000;
        var main_movie_index = <?php echo $data->main_movie_index; ?>;
